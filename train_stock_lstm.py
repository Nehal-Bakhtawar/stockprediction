...
# Handle TensorFlow version differences without triggering IDE import warnings
...
def get_data():
    ...
    def download_data():
        ...
        with open(c.names_file, 'r') as f:
            stocks_list = [l.strip() for l in f.readlines()]
            
        print(f"Fetching data for {len(stocks_list)} stocks...")
        by_stock = {}
        for s in stocks_list:
            try:
                # Use a small start buffer to handle normalization length
                df = yf.download(s, start=c.start - timedelta(days=c.normalize_std_len*2), end=c.end)
        ...
        for stock in by_stock:
            ...
            for rec_date in (c.start + timedelta(days=n) for n in range((c.end-c.start).days)):
                ...
                try:
                    # Match date exactly or closest before
                    if rec_date in df.index:
                        ac = df.loc[rec_date]['Adj Close']
                        if isinstance(ac, pd.Series):
                            ac = ac.iloc[0]
...
class StockLSTM(object):
    ...
    def __init__(self, is_training, config):
        ...
        if is_training and config.keep_prob < 1:
            inputs = [tf.nn.dropout(input_, rate=1 - config.keep_prob) for input_ in inputs]

        outputs, states = tf.nn.static_rnn(cell, inputs, initial_state=self._initial_state)
        rnn_output = tf.reshape(tf.concat(outputs, 1), [-1, size])
...
if __name__ == '__main__':
    ...
    ma = inspect.getfullargspec(main)
